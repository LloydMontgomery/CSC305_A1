...
// there may be some differences on Windows
...
jMat4::jMat4( Type type, jNorm3 u, jFlt theta )
{
    if(type!=mRotateA)
    {
        std::cerr << "Ambiguous matrix type, abort.\n";
        abort();
    }
...
{
    identity();
    if (type != mBetween) std::cerr << "Must be mBetween type\n";
	else {
		jFlt scalef;
		jVec2 trn;
		jFlt angle;
		...
		if (nearZero(s*t)) {
            std::cerr << "Between not worked - zero length\n";
			return;
		} else {
